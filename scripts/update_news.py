...
def main() -> int:
    ...
    if archive_path.exists() and archive_path.stat().st_size > 0:
        ...
        try:
            ...
            # 2. 提取公共信息和庞大的 items 数组
            generated_at = data.get("generated_at", "")
            items = data.get("items", [])
            
            # 3. 设置每个压缩包存放多少条数据 (50000条压缩后通常只有 1-2 MB)
            chunk_size = 50000 
            ...
            for i in range(0, len(items), chunk_size):
                ...
                # 使用 output_dir 确保路径正确
                output_filename = output_dir / f'archive_part_{part_num}.json.gz'
                
                # 将切分出来的数据重新组装成字典，保留生成时间，方便以后读取
                chunk_data = {
                    "generated_at": generated_at,
                    "part": part_num,
                    "total_items_in_part": len(chunk_items),
                    "items": chunk_items
                }
                
                # 写入 .json.gz 压缩文件
                with gzip.open(output_filename, 'wt', encoding='utf-8') as gz_file:
                    json.dump(chunk_data, gz_file, ensure_ascii=False)
                    
                print(f"已生成压缩文件: {output_filename}")
                    
            # 5. 【极其重要】删除原本的超大 JSON 文件
            archive_path.unlink()  # 使用 pathlib 的 unlink 删除文件
            print(f"清理完毕，已删除原始文件: {archive_path}")
            
        except Exception as e:
            print(f"拆分/压缩文件时发生错误: {e}")
